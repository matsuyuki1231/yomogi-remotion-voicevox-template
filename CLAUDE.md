...
#### 現在のフォーマット: お会計・査定型（この生活、いくら？）

`config/script.yaml` が生活サーバー版。過去のフォーマットは `config/archive/` に
文言のみ残してあり、対応コンポーネントは削除済み（復元する場合は `format-*-v1` タグを参照）。

冒頭で「この生活、ぜんぶでいくらだと思う？」と投げ、マイクラ映像に映る暮らしを
現実のお金に換算して査定していく。車→土地→家→店→社長→釣り三昧と1項目1秒で
積み上げて総額5,785万円に到達した直後、「それが、ぜんぶタダ」で0円に叩き落とし、
そこで初めて「よもぎサーバーの生活サーバー」と明かす。

離脱防止の装置を3つ同時に走らせるのが肝：

| 装置 | 役割 |
|------|------|
| 金額メーター（`MeterPlate`） | 常時回り続けるので間を作れない。次の加算を待たせる |
| 値札スタンプ（`ItemStamp`） | 1項目1カットで画面が1秒ごとに変わる |
| 総額の予感 | 「いくらになるのか」の好奇心ギャップが結果発表まで引っぱる |

値段当ては視聴者が頭の中で勝手に参加してしまう鉄板ジャンルで、締めの
「いくらだと思った？」が査定額のコメント書き込みを誘発しつつ冒頭に戻ってループする。
**前半20秒はサーバー名を一切出さない**ので、一般のマイクラ視聴者への釣りとして機能し、
宣伝が来る頃には離脱ポイントを通り過ぎている。

**`priceAdd` の合計と `priceTotal` / `priceZeroStrike` の数字は必ず一致させること。**
項目を増減したら総額表示も直す。現実換算の金額はあくまで「ざっくり査定」のギャグだが、
サーバー側の事実（何ができるか）は `docs/yomogi` で裏を取ったものだけにする。
BGMのsegments切り替え位置（`fromLineId`）は0円落としの行に合わせること。

#### スクリプトのフィールド（price*）

| フィールド | 用途 |
|-----------|------|
| `priceHook` / `priceHookSub` | 冒頭の巨大フック。改行は `\n` で**明示する**（自動折り返しは崩れる） |
| `priceStart` | 「査定スタート」バッジ。メーターが ¥0 で出現する行 |
| `priceItem` / `priceTag` / `priceAdd` | 査定項目・値札の金額表示・メーターへの加算額（円） |
| `priceDrum` | ドラムロール行。メーターを震わせて結果発表を溜める |
| `priceTotal` / `priceTotalSub` | 総額発表の巨大数字とバッジ |
| `priceZero` / `priceZeroStrike` | 0円スタンプと、打ち消す金額 |
| `priceReveal` / `priceRevealSub` | リビール帯（宣伝への転換点） |
| `priceCta` | 検索バー風CTA（文字がタイプされる) |
| `priceBait` | コメント誘発リボン |

メーターは `priceStart` の行から `priceTotal` の直前まで、合いの手の行でも
出しっぱなしになる（Main側で自動制御）。加算はカット頭14フレームで回りきる。

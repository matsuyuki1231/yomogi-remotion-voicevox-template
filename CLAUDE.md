...
1行目の質問「いま、何してたんですか？」への答えが「会社の決算です。社員が6人いて」なので、
視聴者は"何を見せられているのか"が分からないまま引き込まれる（＝1秒フック）。
以降は**1人1カット**で答えがどんどんおかしくなる（無人店／釣り275種／畑／車／近距離VC）。

常設の**《取材 n人目》ドット列は7個**あり、6人目まで埋まったところで止まる。
「7人目は誰なんだ」という open loop を冒頭のヘッダ帯（「街の人7人に聞いてみた」）から張り、
最終行で**「7人目は、あなた」**と視聴者に向けて回収する。この1枚が
ループ（冒頭の質問に戻る）とコメント誘発（「いま、何してた？」）を同時に満たすのが要点。

転換は「ここ、入るのにいくらかかるの？」→**全員「0円」**の取材終了スラム（白フラッシュ）。
ここで赤「REC」→緑「取材終了」にトーンが変わり、続く id15「この街、どこにあるの？」が
視聴者の疑問を代弁する形で宣伝への導線になる（`InterviewHud.tsx`）。

**この型はサーバー名を id16 まで伏せる**。コメント返信型とは逆で、
...
BGMのsegments切り替え位置（`fromLineId`）は取材終了の行（`id14`）に合わせる。

...
| 背景 | Frutiger Aero の壁紙（青空＋草原＋泡）。`AeroBackground` がCSSで描画 |
| キャラクター立ち絵 | **使わない** |

#### 現在のフォーマット: Frutiger Aero型

`config/script.living-server-aero.yaml` が現行のスクリプト。過去のフォーマットは
`config/archive/` に文言のみ残してあり、対応コンポーネントは削除済み
（復元する場合は `format-*-v1` タグを参照）。

前半（宣伝ゼロ）で2000年代のツヤツヤUIを懐かしませ、「Frutiger Aero という名前がある」
という豆知識で引きつけてから、後半で生活サーバーにつなぐ構成。
`aeroFlat` で一度ツヤを消し、`aeroFlare` で取り戻す落差が転換点になっている。

前半の画面（起動スプラッシュ・デスクトップ・ウィンドウ枠・泡）は
外部画像を一切使わず `src/components/AeroOS.tsx` がCSSで描画している。

#### 動画素材（visual）

- 後半（リビール以降）は全セリフに動画素材を挿入する
- 前半の懐かしパートは `visual` を書かず、Aeroの壁紙をそのまま見せる
- `aeroWindow` を指定したセリフは、映像がウィンドウ枠の中に嵌め込まれる
  （枠の位置は `AERO_WINDOW`。字幕と重ならないよう下端は 1370px で止めている）
...
字幕のアウトラインは深い青（`#0b4f88`）で固定する。空の壁紙にもマイクラ映像にも乗るため。

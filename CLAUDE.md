...
| ファイル | 中身 | HUD |
|----------|------|-----|
| `config/script.living-server-shop.yaml` | **生活サーバー版・テレビショッピング/通販型（最新）** | `ShopHud` |
...
（`shop*` → 通販型、`court*` → 裁判型、どちらもなければ報道型）、
`ShopHud` / `CourtHud` / `NewsHud`（背景・暗幕・常設UI・字幕抑止まで含めて）を切り替える。
...
**裁判型（`CourtHud`）と報道型（`NewsHud`）は削除していない** — 直近の生活サーバー版と
人狼版の現行フォーマットがこれらを使っているため。

#### 現在のフォーマット: テレビショッピング・通販型（この暮らし、まるごと通販）

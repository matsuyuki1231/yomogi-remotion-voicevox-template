...
#### 現在のフォーマット: 3択・制限時間クイズ型（このゲーム、何問わかる？）

`config/script.yaml` が生活サーバー版。過去のフォーマットは `config/archive/` に
文言のみ残してあり、対応コンポーネントは削除済み（復元する場合は `format-*-v1` タグを参照。
偽ライブ配信型＝`config/archive/script.living-server-livestream.yaml` / `format-livestream-v1`、
お会計・査定型＝`config/archive/script.living-server-price.yaml` / `format-price-v1`）。

視聴者参加型のクイズ。「このゲーム、何問わかる？」と投げ、3択＋カウントダウンリング（3→2→1）で
視聴者に答えさせる（`QuizHud.tsx`）。正解が積み上がるほど生活サーバーの機能が明かされ
（Q1マイクラ→Q2釣り275種類→Q3自分の店→Q4参加費0円）、最終問題のあと「よもぎサーバーの
...
| 装置 | 役割 |
|------|------|
| カウントダウンリング（`CountdownRing`） | 出題ごとに時間が減る＝視聴者が答える強制力。出題行の span 全体で回る |
| 3択カードの点灯リビール（`ChoiceList`） | 1問1カットで切り替わり、解答行で正解が緑に光る＋「正解」スタンプ |
| スコアカウンター（`ScoreBadge`） | 「ここまで◯問」で達成感を積み、締めで自分の点数を問う |

**1問＝出題行＋解答行の2行構成**。出題行は `quizTimer:true`（リングが回る）、解答行は
`quizAnswerReveal:true`（正解が光る＋スコア加算）。`quizChoices` は**両方の行に同じ内容**を書く
（カードを2カットにわたって見せるため）。`quizAnswer` の正解位置はなるべく散らす（毎回同じ位置だと
当てずっぽうで当たる）。問題番号・進行（◯/4）・スコアは Main が自動集計する。

**前半 Q1〜Q3 はサーバー名を出さない**。「この街／家／店は何？」の当てものとして釣り、
`Q1` で「正解はマイクラ」のギャップを一発置く。字幕は情報が二重になるのを避けるため
**検索CTAの行だけ**出す（設問文・選択肢・各バナーが画面テロップを兼ねる）。
サーバー側の事実（無料・24時間・土地/家/店/会社/釣り/車）だけを `docs/yomogi` で裏を取ること。
BGMのsegments切り替え位置（`fromLineId`）はリビールの行（`id10`）に合わせる。

#### スクリプトのフィールド（quiz*）

| フィールド | 用途 |
|-----------|------|
| `quizHook` / `quizHookSub` | 冒頭の巨大フック。改行は `\n` で**明示する**（自動折り返しは崩れる） |
| `quizNo` | 問題番号バッジ（例: `Q1`）。この値があると進行「◯/4」も出る |
| `quizQ` | 設問文（出題行・解答行の両方に書く。解答行では小さく上に残す） |
| `quizChoices` | 3択（A/B/C の文字列配列。出題行・解答行の両方に同じ内容） |
| `quizAnswer` | 正解の選択肢インデックス（0始まり） |
| `quizTimer` | 出題フェーズ。カウントダウンリングを回す |
| `quizAnswerReveal` | 解答フェーズ。正解を緑に光らせて「正解」スタンプ＋スコア加算 |
| `quizReveal` / `quizRevealSub` | リビール帯（宣伝への転換点） |
| `quizCta` | 検索バー風CTA（文字がタイプされる） |
| `quizResult` | 結果＝コメント誘発リボン（冒頭に戻してループ） |
...
字幕は `bottomOffset: 150` で最下部に寄せる。3択カード（最下段 y≒1390）の下に置くため。
`displayText` は **1〜2行に収まる長さ**にする。3行になると画面が重い。

クイズ型では設問文・選択肢・各バナーが画面テロップを兼ねるので、**字幕は検索CTAの行だけ**出す
（`hidesSubtitle` が `quizHook` / `quizQ` / `quizReveal` / `quizResult` の行で字幕を抑止する）。

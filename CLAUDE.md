...
| 背景 | 全編に実映像（マイクラ素材）。素材がない行だけ `ReplyBackdrop` / `CourtBackdrop` / `NewsBackdrop` のCSS背景 |
| キャラクター立ち絵 | **使わない** |
...
| ファイル | 中身 | HUD |
|----------|------|-----|
| `config/script.living-server-reply.yaml` | **生活サーバー版・コメント返信/反論処理型（最新）** | `ReplyHud` |
...
`ReplyHud` / `ShopHud` / `CourtHud` / `NewsHud`（背景・暗幕・常設UI・字幕抑止まで含めて）を切り替える。
...
**通販型（`ShopHud`）・裁判型（`CourtHud`）・報道型（`NewsHud`）は削除していない** —
直近の生活サーバー版と人狼版がこれらを使っているため。

#### 現在のフォーマット: コメント返信・反論処理型（来た質問、全部答える）
...
- 映像の上には自動で暗幕（`ReplyScrim` / `ShopScrim` / `CourtScrim` / `NewsScrim`）がかかる
  （テロップを読ませるため）
...
- コメント返信型のように「振り（質問）→ 受け（回答）」で2行1組にする場合は、
  振りの行を `-4`、受けの行を `-3` にして組の中だけ詰める
- 効果音（SE）は**全セリフに入れる**

...
#### 現在のフォーマット: 偽ライブ配信・バズ中継型（今バズってる深夜配信を装う）

`config/script.yaml` が生活サーバー版。過去のフォーマットは `config/archive/` に
文言のみ残してあり、対応コンポーネントは削除済み（お会計・査定型を含む。復元する場合は
`format-*-v1` タグを参照。査定型の文言は `config/archive/script.living-server-price.yaml`）。

マイクラ実映像の上に「ライブ配信のUI」を丸ごと被せる（`LiveHud.tsx`）。冒頭で
「同接17万人の配信、何これ？」と投げ、暮らし（家→土地→店→車→社長→釣り）が次々映るたびに
コメントが沸き、同時接続カウンターが伸び続ける。「これ何の配信？」→「マイクラなのだ」で
コメント爆発、そこで初めて「よもぎサーバーの生活サーバー」と明かす。オチは同接の正体回収＝
「これ"配信"じゃない。全員が自分でプレイしてる人たち」。参加費0円で締める。

離脱防止の装置を3つ同時に走らせるのが肝（余白を作らせない）：

| 装置 | 役割 |
|------|------|
| 同接カウンター（`LiveBar`） | 常時回り続ける。数字が伸びる快感＋社会的証明。カット頭14フレームで回る |
| 流れるコメント（`LiveCommentLayer`） | 行境界をまたいで下から上へ流れ続ける。画面が常に動き、視聴者の疑問を先回りで代弁 |
| スーパーチャット（`SuperChatCard`） | 1カット1回、右から飛び込んでテンポを刻む |

流れるコメントは**Sequence の外**（`AbsoluteFill` 直下）に1本のタイムラインとして描く。
これで `useCurrentFrame` が動画全体の通しフレームになり、コメントが行をまたいで途切れない。
各行の `comments` は行頭から少しずつずらして投入される（Main側で自動スケジュール）。

**前半 id1〜id9 はサーバー名もマイクラも一切出さない**（フェイクアウトの生命線）。一般の
ショート視聴者を「新作ゲーム？メタバース？」と釣り、宣伝が来る頃には離脱ポイントを通り過ぎている。
`id6` のコメントに「え、マイクラ？」の伏線を1つだけ置き、`id10` の爆発の前フリにする。
締めの「同接、何人だと思った？」が冒頭の「同接17万人」に戻ってループし、コメント欄への
書き込みを誘発する。

**同接の数字は「配信の演出」というギャグ装置。`id13` で「実は配信じゃない」と正体を回収する。**
サーバー側の事実（無料・24時間・土地/家/店/会社/釣り/車）だけを `docs/yomogi` で裏を取ること。
BGMのsegments切り替え位置（`fromLineId`）はリビールの行（`id11`）に合わせる。

#### スクリプトのフィールド（live*）

| フィールド | 用途 |
|-----------|------|
| `liveHook` / `liveHookSub` | 冒頭の巨大フック。改行は `\n` で**明示する**（自動折り返しは崩れる） |
| `liveViewers` | 同時接続の目標値（数値）。値がある行の間だけカウンターが出て from→to へ回る |
| `liveTitle` | LIVEバーの配信タイトル（省略時は「深夜の生活配信」） |
| `comments` | この行で投入する流れるコメント（文字列の配列） |
| `pinned` | ピン留めコメント（冒頭フックの補強。3行ぶん出しっぱなし） |
| `superChat` | スパチャ（`{name, amount, text}`）。右から飛び込む投げ銭カード |
| `liveReaction` | コメント爆発の一撃（例: `マイクラ!?`） |
| `liveReveal` / `liveRevealSub` | リビール帯（宣伝への転換点） |
| `liveCta` | 検索バー風CTA（文字がタイプされる） |
| `liveBait` | コメント誘発リボン（冒頭の同接に戻してループ） |

カウンターは `liveViewers` を持つ行の間だけ出る（リビール＝`id11` 以降は値を持たせず消す＝
「配信」の建前が崩れる）。`liveViewers` を増減したら数字の階段が単調増加になるよう直すこと。
...
字幕は `bottomOffset: 150` で最下部に寄せる。スパチャカード（y=1240）と流れるコメントの
下端を避けつつ最下部に置くため。`displayText` は **1〜2行に収まる長さ**にする。3行になると画面が重い。

デカ文字テロップ（`liveHook` / `liveReveal` / `liveBait`）を出した行は、同じことを二重に
見せないため字幕を出さない（Main側で制御）。`liveReaction`（`マイクラ!?`）は話者のセリフとは
別のコメント爆発なので、字幕は出したままにする。

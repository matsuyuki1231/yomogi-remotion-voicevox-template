...
めたんとずんだもんが1件ずつ返信していく。常設の**《未回答 5件》カウンター**が
1件ずつ減っていくので「0件になったら終わるのか」で最後まで引っぱれる。
最後の質問「で、いくらかかるの？」への回答「0円」で未回答0件＝**回答完了スラム**
（白フラッシュ）を出してトーンを解除し、その直後に**新着1件「で、どこのサーバーなの？」**が
飛び込む（終わったと見せてから引き戻すギャップ）。
視聴者の質問に答える形で正体を明かすので、宣伝が押し売りにならないのがこの型の要点。
...
| 未回答カウンター＋解決バー（`replyPending`） | 5→0。「あと何件」で引っぱり、新着で1に戻る瞬間が最大のギャップ |
...
**前半（id1〜14）はサーバー名を出さない**。一般的な「マイクラ飽きた」論として読めるようにして、
回答完了スラム（id14）で緊張を解いてから、新着コメント（id15）→ `replyReveal`（id16）で正体を明かす。

...
| 背景 | 深い紺のグラデーション＋光の玉。`TriviaBackground` がCSSで描画 |
| キャラクター立ち絵 | **使わない** |

#### 現在のフォーマット: 雑学連発型

同じ TriviaCard を使うスクリプトが2本ある（雑学の中身と宣伝先だけが違う）:
- `config/script.living-server-trivia.yaml` … 生活サーバー宣伝（バナナ/タコ/エッフェル塔/ハチミツ）
- `config/script.werewolf-trivia.yaml` … マイクラ人狼宣伝（托卵/擬態/嘘の看破率/「ウソ」という鳥）

宣伝したい対象のスクリプトを `config/script.yaml` にコピーしてから
`npm run sync-script → npm run voices` する。過去のフォーマットは
`config/archive/` に文言のみ残してあり、対応コンポーネントは削除済み
（復元する場合は `format-*-v1` タグを参照）。

人狼版は「だます／見抜く」でそろえた雑学を前ふりにして人狼へ着地させ、
その3（人は嘘を約54％しか見抜けない）を後半で回収する構成。
BGMのsegments切り替え位置（`fromLineId`）はスクリプトを差し替えたら合わせて直すこと。

「知ってると得する雑学5選」の顔で始め、その1〜その4は生活サーバーと**まったく無関係な
雑学**を畳みかける。視聴者はリストの完走目的で残るので、宣伝が来る頃には離脱ポイントを
通り過ぎている。その5で「今のマイクラは会社を作って社長になれる」という
“雑学の顔をした宣伝”に着地し、めたんが「それ宣伝じゃない？」とツッコみ、
ずんだもんが「バレたのだ」と認めることでネタとして成立させる。

雑学ごとにアクセント色が変わる（黄→水→橙→桃→緑）ので、映像素材のない前半でも
画面が単調にならない。前半の画面は外部画像を使わず
`src/components/TriviaCard.tsx` がCSSと絵文字で描画している。

**雑学の中身は必ず裏の取れた事実にすること。** 出典ラベル（`triviaSource`）を出す
フォーマットなので、あやしい俗説を混ぜると動画全体の信頼を落とす。

#### 動画素材（visual）

- 映像素材を使うのは **その5（宣伝パート）だけ**。雑学パートは `visual` を書かない
- 映像の上には自動で暗幕がかかる（テロップを読ませるため）
- 番号バッジ・問い・絵文字は同じ雑学の間ずっと出しっぱなしになる（Main側で持ち越し）。
  映像を大きく見せたい行では `triviaClear: true` で問いと絵文字の持ち越しを打ち切る
...
| pauseAfter（通常） | -3 |
| pauseAfter（バイバイ直前） | 0 |
| pauseAfter（最後のセリフ） | 60 |

- 効果音（SE）は**全セリフに入れる**（最後のバイバイ掛け合いの2行のみ除外）
...
字幕のアウトラインは深い紺（`#080d24`）で固定する。紺の背景にもマイクラ映像にも乗るため。
また太い袋文字だと「……」が四角い塊に見えるので、`displayText` に三点リーダーを使わない
（`text` 側は音声の間として残してよい）。

デカ文字テロップ（`trivia` / `triviaAnswer`）を出した行は、同じことを二重に見せないため
字幕を出さない（Main側で制御）。

#### 技術的注意点（レンダリング品質）

- 動画素材には `<OffthreadVideo>` を使う（`<Video>` はレンダリング時にfpsが落ちる）
- BGMは `<Sequence durationInFrames={bgmTotalFrames}>` で囲む（レンダリング時の音声はみ出し防止）

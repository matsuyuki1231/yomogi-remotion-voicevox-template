...
| 背景 | 全編に実映像（マイクラ素材）。素材がない行だけ `NewsBackdrop` のCSS背景 |
| キャラクター立ち絵 | **使わない** |
...
出現」と中継で騒ぐ。道路・車・営業中の店・畑・会社と"生活の痕跡"が次々見つかり（前半＝緊張・茶番）、
...
| 装置 | 役割 |
|------|------|
| 報道UI（`NewsChrome`） | 速報ヘッダ帯・流れるティッカー・走査線・時計が**常時**動く。画面が止まらない |
| トーン切替（`newsTone`） | リビール以降 赤「速報」→緑「お知らせ」。走査線と赤ビネットも消えて安堵に転じる |
| 下部ニューススーパー（`LowerThird`） | 1カット1本、左からワイプ。字幕の代わりに読ませる |
| LIVE中継バッジ・専門家プレート・続報スタンプ | 報道の"それっぽさ"で茶番を成立させる |

`NewsChrome` は**セリフごとの Sequence の外側**（Main 直下）に置く。グローバルなフレームで
動かさないと、カットのたびにティッカーと時計がリセットされて途切れる。ティッカー文字列は
全行の `newsTicker` を Main が連結して1本にする。

**前半（id1〜7）はサーバー名を出さない**。「この街は何なのか」という好奇心ギャップだけで引っぱり、
`newsCorrection`（訂正スラム）で緊張を解いてから `newsReveal` で正体を明かす。
字幕は情報が二重になるのを避けて**検索CTAの行だけ**出す（ヘッドライン・スーパーが画面テロップを兼ねる）。
サーバー側の事実（無料・24時間・統合版・土地/家/店/会社/釣り275種/車）だけを `docs/yomogi` で裏を取ること。
BGMのsegments切り替え位置（`fromLineId`）はリビールの行（`id8`）に合わせる。
...
それでも `newsLower` は**13文字前後まで**にすると大きく出せる（ラベルのぶん幅が減る）。

#### 動画素材（visual）

- **全行に映像を敷く**。1項目1カットで割り当て、1秒ごとにカットが変わる
- 映像の上には自動で暗幕（`NewsScrim`）がかかる（テロップを読ませるため）
...
報道型ではヘッドライン・ニューススーパー・リビール帯が画面テロップを兼ねるので、**字幕は検索CTAの行だけ**出す
（`hidesSubtitle` が `newsFlash` / `newsLower` / `newsCorrection` / `newsReveal` / `newsResult` の行で字幕を抑止する）。

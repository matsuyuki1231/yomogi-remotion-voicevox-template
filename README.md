...
[fork元](https://github.com/nyanko3141592/remotion-voicevox-template)を改変し、よもぎサーバーの紹介short動画の作成に特化した機能を追加しています。

![解像度](https://img.shields.io/badge/解像度-1080x1920-blue)
![フレームレート](https://img.shields.io/badge/FPS-30-green)

## Vibe Codingによる改変

以下の理由から、このプロジェクトはmatsuyuki1231がClaude Codeを使用したVibe Codingにより改変を行っています。  

 - ローカルのPCのみでの動作を想定しており、第三者による入力操作がないことから、セキュリティに関して考慮する点が少ない
 - fork元がMITライセンスであり、改変が制限されていない
 - 本プロジェクトもMITライセンスを継承するため、AS ISでの公開が可能である

matsuyuki1231は改変されたコードの内容を完全には把握していません。
それゆえ、matsuyuki1231は改変された箇所にかかる技術的詳細についての質問についてはお答えすることができませんので、あらかじめご承知おきください。  
なお例外として、本ファイルについてはmatsuyuki1231が手動で編集を行っています。  

最後に、本セクションの末筆となり大変恐縮ですが、fork元の作者様に深く敬意を表すとともに、感謝申し上げます。
